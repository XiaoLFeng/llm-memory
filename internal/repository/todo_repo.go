...
// 呀~ 通过依赖注入的方式创建仓储对象~ ✨
func NewTodoRepo(dbPath string) *TodoRepo {
	return &TodoRepo{
		dbPath: dbPath,
	}
}
...
func (r *TodoRepo) Delete(ctx context.Context, id int) error {
	return database.OpenWithActionNoReturn(r.dbPath, func(db *database.DB) error {
		return db.DeleteStruct(&types.Todo{ID: id})
	})
}
...
func (r *TodoRepo) FindByID(ctx context.Context, id int) (*types.Todo, error) {
	return database.OpenWithAction(r.dbPath, func(db *database.DB) (*types.Todo, error) {
		var todo types.Todo
		err := db.One("ID", id, &todo)
		if err != nil {
			return nil, err
		}
		return &todo, nil
...
func (r *TodoRepo) FindByStatus(ctx context.Context, status types.TodoStatus) ([]types.Todo, error) {
	return database.OpenWithAction(r.dbPath, func(db *database.DB) ([]types.Todo, error) {
		var todos []types.Todo
		err := db.Find("Status", status, &todos)
		if err != nil {
			return nil, err
		}
		return todos, nil
