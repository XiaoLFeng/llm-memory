...
// 呀~ 构造函数来啦！接收一个 DB 实例~ (´∀｀)
func NewGroupRepo(db *database.DB) *GroupRepo {
	return &GroupRepo{
		db: db,
	}
}
...
func (r *GroupRepo) Delete(ctx context.Context, id int) error {
	...
	err := r.db.Find("GroupID", id, &mappings)
	if err == nil {
		for _, mapping := range mappings {
			_ = r.db.DeleteStruct(&mapping)
		}
	}

	// 再删除组
	group := &types.Group{ID: id}
	return r.db.DeleteStruct(group)
}
...
func (r *GroupRepo) FindByPath(ctx context.Context, path string) (*types.Group, error) {
	...
	err = r.db.One("Path", absPath, &mapping)
	if err != nil {
	...
	// 根据映射的 GroupID 查找组
	return r.FindByID(ctx, mapping.GroupID)
}
...
func (r *GroupRepo) AddPath(ctx context.Context, groupID int, path string) error {
	...
	// 检查路径是否已被其他组占用
	existingGroup, err := r.FindByPath(ctx, absPath)
	if err != nil {
		return err
	}
	if existingGroup != nil && existingGroup.ID != groupID {
	...
	// 更新组
	if err := r.db.Update(group); err != nil {
		return err
	}

	// 创建路径映射
	mapping := types.NewGroupPathMapping(absPath, groupID)
	return r.db.Save(mapping)
}
